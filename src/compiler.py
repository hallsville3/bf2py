...
class Compiler(object):
    ...
    def compile(self, op_level=2, cap=0):
        ...
        self.compiled.append("inputs = list(input('Inputs: '))")
        ...
        while index < len(self.bf):
            chars = [
                "\0" if index + i >= len(self.bf) else self.bf[index + i]
                for i in range(10)
            ]
            char0 = chars[0]
            char1 = chars[1]
                    ...
                    if re.compile("\[->+\+<+\]").match(expression):
                        ...
                        if cap:
                            self.compiled.append(
                                indent * indent_count
                                + "data[ptr + {}] = (data[ptr + {}] + data[ptr]) % {}".format(
                                    count, count, cap
                                )
                            )
                        else:
                            self.compiled.append(
                                indent * indent_count
                                + "data[ptr + {}] += data[ptr]".format(count)
                            )
                        self.compiled.append(indent * indent_count + "data[ptr] = 0")
                        self.line_count += 2
                        ...
                        counts = [
                            group.count(">")
                            for group in re.compile(">+").findall(expression)
                        ]
                        if cap:
                            self.compiled.append(
                                indent * indent_count
                                + "data[ptr + {}] = (data[ptr + {}] + data[ptr]) % {}".format(
                                    counts[0], counts[0], cap
                                )
                            )
                            self.compiled.append(
                                indent * indent_count
                                + "data[ptr + {}] = (data[ptr + {}] + data[ptr]) % {}".format(
                                    sum(counts), sum(counts), cap
                                )
                            )
                        else:
                            self.compiled.append(
                                indent * indent_count
                                + "data[ptr + {}] += data[ptr]".format(counts[0])
                            )
                            self.compiled.append(
                                indent * indent_count
                                + "data[ptr + {}] += data[ptr]".format(sum(counts))
                            )
            ...
            if char0 in "+-":
                ...
                if op_level == 0 or char1 not in "+-" and sum_balance != 0:
                    ...
                    if sum_balance > 0:
                        ...
                        if cap:
                            ...
                            self.compiled.append(
                                indent * indent_count
                                + "data[ptr] -= {}".format(abs(sum_balance))
                            )
                ...
                if op_level == 0 or char1 not in "<>" and location_balance != 0:
                    ...
                    if location_balance > 0:
                        ...
                        self.compiled.append(
                            indent * indent_count
                            + "ptr -= {}".format(abs(location_balance))
                        )
                ...
                if cap:
                    self.compiled.append(
                        indent * indent_count
                        + f"data[ptr] = (ord(inputs.pop()) % {cap}) if inputs else data[ptr]"
                    )
                    self.line_count += 1
                else:
                    self.compiled.append(
                        indent * indent_count
                        + "data[ptr] = ord(inputs.pop()) if inputs else data[ptr]"
                    )
                    self.line_count += 1
